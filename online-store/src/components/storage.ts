...
export function getLocalStorage() {
  const filters = JSON.parse(localStorage.getItem('filter') as string);
  const sort = JSON.parse(localStorage.getItem('sort') as string) as string;
  const inputSearch = JSON.parse(
    localStorage.getItem('search') as string
  ) as string;
  (car.productOfCar =
    new Set(JSON.parse(localStorage.getItem('car') as string)) || new Set()),
    (filter = {
      ...
      brand:
        new Set(JSON.parse(localStorage.getItem('brand') as string)) ||
        new Set(),
      category:
        new Set(JSON.parse(localStorage.getItem('category') as string)) ||
        new Set(),
      age:
        new Set(JSON.parse(localStorage.getItem('age') as string)) || new Set(),
