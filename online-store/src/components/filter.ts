  ...
  if (listing) {
    listing.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).closest('.card')) {
        const clickedCardId: string = (e.target as HTMLElement)
          .closest('.card')
...
export const addSearchClickHandler = () => {
  ...
  inputSearch?.addEventListener('keyup', (e) => {
    cards.filterProducts(
      filterSearch((e.target as HTMLInputElement).value, cards.catalog),
      car.productOfCar
    );
  });
...
function filterSearch(val: string, list: IProduct[]) {
  return list.filter(
    (elem) => ~elem.name.toLowerCase().indexOf(val.toLowerCase())
  );
}
...
export const addBrandClickHandler = () => {
  const brand = document.querySelector('.list-brand');
  if (brand) {
...
export const addCategoryClickHandler = () => {
  const category = document.querySelector('.list-catigory');
  if (category) {
...
export const addSaleClickHandler = () => {
  const age = document.querySelector('.sale-input');
  if (age) {
    age.addEventListener('click', (e) => {
...
export const addResetSettingClickHandler = () => {
  const resetSettings = document.querySelector(
    '.reset-settings'
...
function clearFilters() {
  const rangeAmount = document.getElementById(
    'range-amount'
  ) as noUiSlider.target;
  const rangePrice = document.getElementById(
    'range-price'
  ) as noUiSlider.target;
