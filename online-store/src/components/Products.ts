...
export class Products {
  ...
  filterProducts(data: IProduct[], productOfCar: Set<string>) {
    ...
    this.curProductsOnPage = data.filter((elem: IProduct) => {
      return (
        elem.price >= filter.minPrice &&
        elem.price <= filter.maxPrice &&
        elem.amount >= filter.minAmount &&
        elem.amount <= filter.maxAmount
      );
    });
    ...
    if (filter.onlySale) {
      this.curProductsOnPage = this.curProductsOnPage.filter(
        (elem: IProduct) => filter.onlySale == elem.sale
      );
    }
